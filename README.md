...
Cada execução salva instantâneos específicos para a contagem de núcleos configurada (`DEFAULT_NUM_CORES`):
- `dados_graficos/csv/metricas_<N>cores.csv`
- `dados_graficos/reports/relatorio_metricas_<N>cores.txt`
onde `<N>` é o número efetivo de cores usado na coleta.
...
8. `test-metrics` - Geração completa de métricas/cache/relatórios
