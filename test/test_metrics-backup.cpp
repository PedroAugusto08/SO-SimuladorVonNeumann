...
PolicyMetrics run_policy(const std::string& policy,
    ...
    try {
        ...
        if (policy == "RR")
        {
            ...
            metrics.processes_failed += (scheduler.get_total_count() - scheduler.get_finished_count());
        ...
        else if (policy == "FCFS")
        {
            ...
            // Ensure cores complete any running processes (drain) - schedule short burst
            for (int i = 0; i < 10000 && scheduler.has_pending_processes(); ++i) {
                scheduler.schedule_cycle();
            }
            ...
            {
                // Dump state: basic info about ready queue/cores
                {
                    auto& rq = scheduler.get_ready_queue();
                    auto& cores = scheduler.get_cores();
                    std::cerr << "[DUMP FCFS] ready=" << rq.size() << " finished=" << scheduler.get_finished_count() << " total=" << scheduler.get_total_count() << "\n";
                    for (auto& core : cores) {
                        if (core->get_current_process()) {
                            std::cerr << "  core=" << core->get_id() << " proc=P" << core->get_current_process()->pid << "\n";
                        }
                    }
                }
            ...
            {
                {
                    auto& rq = scheduler.get_ready_queue();
                    auto& cores = scheduler.get_cores();
                    std::cerr << "[DUMP SJN] ready=" << rq.size() << " finished=" << scheduler.get_finished_count() << " total=" << scheduler.get_total_count() << "\n";
                    for (auto& core : cores) {
                        if (core->get_current_process()) {
                            std::cerr << "  core=" << core->get_id() << " proc=P" << core->get_current_process()->pid << "\n";
                        }
...
int main() {
    // Configure logging level from environment variable SIM_LOG_LEVEL (not available here)
    // If a logging facility is added, use it; otherwise, no-op.
    std::cout << "\n╔══════════════════════════════════════════════════════════════╗\n";
