...
// Runtime-overridable global max cycles (default to MAX_CYCLES)
static int GLOBAL_MAX_CYCLES = MAX_CYCLES;
...
int compute_cycle_budget(int num_cores, size_t workload_count) {
    ...
    // Ajusta pelo número de workloads — usa GLOBAL_MAX_CYCLES como base
    const double budget = static_cast<double>(GLOBAL_MAX_CYCLES) * workload_scale;
...
int main() {
    ...
    // Permite sobrescrever o limite global de MAX_CYCLES via variável de ambiente
    if (const char* max_cycles_env = std::getenv("SIM_MAX_CYCLES")) {
        try {
            GLOBAL_MAX_CYCLES = std::stoi(max_cycles_env);
        } catch (...) {
            std::cerr << "⚠️  SIM_MAX_CYCLES inválido: " << max_cycles_env << " - ignorando\n";
        }
    }
