...
MetricsResult print_statistics_rr(const std::string& policy, const RoundRobinScheduler::Statistics& stats) {
    ...
    std::cout << "  • Tempo Médio de Espera:     " << stats.avg_wait_time << " ms\n";
    std::cout << "  • Tempo Médio de Execução:   " << stats.avg_turnaround_time << " ms\n";
...
int main() {
    ...
    if (report.is_open()) {
        ...
        for (const auto& r : results) {
            ...
            report << "  • Tempo Médio de Espera:      " << r.avg_wait_time << " ms\n";
            report << "  • Tempo Médio de Execução:    " << r.avg_turnaround_time << " ms\n";
        ...
        auto max_throughput = *std::max_element(results.begin(), results.end(),
         ...
         report << "  🏆 Melhor Tempo de Espera:     " << min_wait.policy 
             << " (" << std::fixed << std::setprecision(2) << min_wait.avg_wait_time << " ms)\n";
         report << "  🏆 Melhor Tempo de Execução:   " << min_exec.policy
             << " (" << std::fixed << std::setprecision(2) << min_exec.avg_turnaround_time << " ms)\n";
